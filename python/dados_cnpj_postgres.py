...
# get host from environment variable if available

tipo_banco = "postgres"
dbname = os.getenv("DATABASE_NAME")
username = os.getenv("DATABASE_USER")
password = os.getenv("DATABASE_PASSWORD")
host = os.getenv("DATABASE_IP") + ":" + os.getenv("DATABASE_PORT")

pasta_compactados = r"dados-publicos-zip"
pasta_saida = r"dados-publicos"  # esta pasta deve estar vazia.
dataReferencia = "02/08/2023"  # input('Data de referência da base dd/mm/aaaa: ')

# resp = input(
#     f"Isto irá CRIAR TABELAS ou REESCREVER TABELAS no database {dbname.upper()} no servidor {tipo_banco} {host} e MODIFICAR a pasta {pasta_saida}. Deseja prosseguir? (S/N)?"
# )
...
def carregaTipo(nome_tabela, tipo, colunas):
    ...
    for arq in arquivos:
        ...
        ddf.to_sql(
            ...
            str(engine.url),
            index=None,
            if_exists="append",  # parallel=True, #method='multi', chunksize=1000,
